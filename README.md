...
- **Knows who said what** — On-device speaker detection tells the people on the call apart. Name each voice once from short clips, and the transcript, reports, and coaching all use real names. Turn on **Remember voices** and Parrot suggests who's talking on the next call. Free and fully local. (Meetily charges $10/month for diarization; Anarlog $15/month, via the cloud.) With "Remember voices" on, a voice heard before is recognized: a very close match is named for you (with an undo), a looser one is a one-click "Sounds like X? Confirm". Calendar invitees appear as one-click chips, break ties between remembered voices, and fill in the last unnamed voice as a "Probably X?" guess
...
- **Two meetings in one recording?** — Pause the player where the second call began, right-click the track, and **Split Meeting** cuts the audio, transcript, and insights into two meetings, each with its own speaker detection and report. Or click the scissors and type a timestamp
...
- **Knows your calendar** — Opt in and every recording is named after its calendar event, remembers the invitees, and briefs the Copilot with the agenda. Ask for a reminder two minutes before each video call, or let Parrot record scheduled Meet, Zoom, and Teams calls on its own and stop when the call goes quiet. Read from the Mac's Calendar app, no Google account needed
...
- **Pre-call brief** — an optional one-liner on the Today pane before you hit record ("Call with Westfield PM about AC replacement") so the copilot has context from second one.
...
Parrot/
  ...
  Views/
    ...
    DashboardView.swift        # Today pane: now/next call, record, today's recordings
