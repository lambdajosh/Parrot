...
| `Parrot/ProfileTest.swift` | 1224 | `--profile-test`: headless logic harness, ~60 assertions |
...
| File | L | Purpose |
|---|---|---|
| `Models/Meeting.swift` | 410 | `Meeting` record + `MeetingStatus` lifecycle + per-speaker names/embeddings + truncate/split |
...
| File | L | Purpose |
|---|---|---|
| `Services/RecordingManager.swift` | 834 | Orchestrates a recording session end-to-end; the hub |
...
| `Services/SpeakerProfileStore.swift` | 116 | Voiceprint matching (cosine ≥ 0.7), remember/forget for named voices |
...
| `Views/MeetingDetailView.swift` | 1227 | Post-call tabs: transcript, insights, report; speaker naming popover + confirm card |
...
| `Views/SettingsView.swift` | 1016 | All settings sections, provider keys, KB docs |
