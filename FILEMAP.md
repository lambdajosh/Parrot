...
| `Parrot/ProfileTest.swift` | 1253 | `--profile-test`: headless logic harness, ~60 assertions |
...
| File | L | Purpose |
|---|---|---|
| `Models/Meeting.swift` | 426 | `Meeting` record + `MeetingStatus` lifecycle + per-speaker names/embeddings + truncate/split |
...
| File | L | Purpose |
|---|---|---|
| `Services/RecordingManager.swift` | 835 | Orchestrates a recording session end-to-end; the hub |
...
| `Views/MeetingDetailView.swift` | 1250 | Post-call tabs: transcript, insights, report; speaker naming popover + confirm card |
